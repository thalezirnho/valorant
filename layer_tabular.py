...
def valorant_layer_tabular(matches=None):
    ...
    # Reading metadata
    logging.info('Reading metadata for each table...')
    # Check if exists
    if not os.path.exists("utils/json_metadata.json"):
        blob = bucket.blob(f"utils/json_metadata.json")
        blob.download_to_filename(f'utils/json_metadata.json')
    # Reading
    with open('utils/json_metadata.json') as f:
        data = json.load(f)


    count = 1
    
    logging.info('Starting matches iteration...')
    ...
    for matchid in matches:
        ...
        # Check if exists in local folder, if don't download it from storage
        if not os.path.exists(f"match/{matchid}.json"):
            blob = bucket.blob(f"match/{matchid}.json")
            blob.download_to_filename(f'match/{matchid}.json')
            print(f'Download {matchid} completed...')
            
        logging.info(f'Reading {matchid}.json from local match folder...')
        with open(f'match/{matchid}.json', 'r') as f:
            match = json.load(f)

        logging.info(f'Starting proccess to transform all json into tables...')
        ...
        logging.info(f'Iterating Players section...')
        for player in all_players:

            puuid = player['puuid']

            for key in json_players_keys:
                logging.info(f'Transforming subsections into variables....')

                # IMPORTANT! CHANGE THIS IMEDIATLY
                exec(f"player_{key} = player.pop('{key}')")

            logging.info(f'Creating df_player...')
            temp_player = pd.json_normalize(player)
            temp_player['matchid'] = matchid
            df_player = pd.concat([df_player, temp_player])

            logging.info(f'Creating df_player_ability_casts...')
            ...
            logging.info(f'Creating df_player_assets...')
            ...
            logging.info(f'Creating df_player_behavior...')
            ...
            logging.info(f'Creating df_player_economy...')
            ...
            logging.info(f'Creating df_player_platform...')
            ...
            logging.info(f'Creating df_player_session_playtime...')
            ...
            logging.info(f'Creating  df_player_stats...')
        ...
        ## Teams
        logging.info(f'Creating df_teams...')
        ...
        logging.info(f'Iterating rounds...')
        for rnd in rounds:
            ...
            # Plant Events
            logging.info(f'Creating df_round_plant_events..')
            ...
            if player_locations_on_plant is not None:
                logging.info(f'Creating df_player_locations_on_plant')
            ...
            # Defuse Events
            logging.info(f'Creating df_round_defuse_events...')
            ...
            if player_locations_on_defuse is not None:
                logging.info(f'Creating df_player_locations_on_defuse...')
            ...
            logging.info(f'Creating temp_round_player_stats...')
            ...
            for element in round_player_stats:
                ...
                if len(damage_events) >= 1:
                    logging.info(f'Creating df_round_player_damage_events...')
        ...
        logging.info(f'Iterating into kills events...')
        for kill in kills:
        ...
        logging.info(f'Reinforcing all dtypes for all dataframes....')
        # Adjusting datatype
        ...
        logging.info(f'Starting Big Query Ingestion...')
